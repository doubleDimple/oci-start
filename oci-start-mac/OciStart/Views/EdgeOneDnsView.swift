...
/// Native EdgeOne DNS (Web: /dns/edgeone)
struct EdgeOneDnsView: View {
        ...
        .sheet(isPresented: $showAdd) {
            if let z = selectedZone {
                DnsEditSheet(
                    ...
                    initial: nil,
                    showProxied: false,
                    onSave: { type, name, content, ttl, _ in
                        try await appState.network.eoAddRecord(
        ...
        .sheet(item: $editRecord) { rec in
            if let z = selectedZone {
                DnsEditSheet(
                    ...
                    showProxied: false,
                    onSave: { type, name, content, ttl, _ in
                        try await appState.network.eoUpdateRecord(
        ...
        .alert(isPresented: $showDelete) {
            Alert(
                ...
                primaryButton: .destructive(Text("删除")) {
                    guard let r = deleteRecord else { return }
                    Task {
                        do {
                            let res = try await appState.network.eoDeleteRecord(
                                baseURL: appState.serverURL, recordId: r.id)
    ...
    private var toolbar: some ToolbarContent {
        ...
        ToolbarItem {
            Button(action: { if selectedZone != nil { showAdd = true } }) {
    ...
    private func loadZones() async {
        ...
        do {
            let arr = try await appState.network.eoZones(baseURL: appState.serverURL)
    ...
    private func loadRecords() async {
        ...
        do {
            let arr = try await appState.network.eoRecords(baseURL: appState.serverURL, zoneId: z.id)
    ...
    private func syncZone() async {
        guard let z = selectedZone else { return }
        do {
            let r = try await appState.network.eoSyncZone(
