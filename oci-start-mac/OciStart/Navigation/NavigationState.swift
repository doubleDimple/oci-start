...
final class NavigationState: ObservableObject {
    ...
    private var leaveGuardOwner: UUID?
    private var leaveGuard: (() -> Bool)?

    func setLeaveGuard(owner: UUID, _ check: @escaping () -> Bool) {
        leaveGuardOwner = owner
        leaveGuard = check
    }

    func removeLeaveGuard(owner: UUID) {
        guard leaveGuardOwner == owner else { return }
        leaveGuardOwner = nil
        leaveGuard = nil
    }

    func canLeaveCurrentPage() -> Bool { leaveGuard?() ?? true }
    ...
    func select(_ nav: NavID) {
        guard nav != selected, canLeaveCurrentPage() else { return }
        performSelection(nav)
    }

    private func performSelection(_ nav: NavID) {
        Task { @MainActor in FloatingMenuDismiss.all() }
        selected = nav
    }

    /// 租户详情 → 实例列表（对齐 Web `/oci/list?tenantId=`）
    func openInstances(parentId: String, regionId: String) {
        guard canLeaveCurrentPage() else { return }
        pendingInstancesFilter = PendingTenantListFilter(
            parentTenantId: parentId,
            regionTenantId: regionId
        )
        performSelection(.instances)
    }
