...
extension TerminalView {
    ...
    func resetFont()
    {
        ...
        let newCols = max(2, Int(getEffectiveWidth(size: frame.size) / cellDimension.width))
        let newRows = max(1, Int(frame.height / cellDimension.height))
    ...
    func processSizeChange (newSize: CGSize) -> Bool {
        ...
        let newCols = max(2, Int (getEffectiveWidth (size: newSize) / cellDimension.width))
