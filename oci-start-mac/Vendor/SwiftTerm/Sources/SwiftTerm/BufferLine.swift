...
public class BufferLine: CustomDebugStringConvertible {
    ...
    {
        let start = max(0, min(startCol, data.count))
        var ec = endCol == -1 ? data.count : max(start, min(endCol, data.count))
        if trimRight {
            ec = max (start, min (ec, getTrimmedLength()))
        }
        var result = ""
        for i in start..<ec {
            let cell = data[i]
            if cell.code == 0 {
                // 1.2.5 stores the second cell of a wide glyph as a NUL with
                // width 1. It is screen padding, not an extra text character.
                // Inspect the previous physical cell even when the requested
                // range starts on this continuation; never skip a real cell.
                let continuation = i > 0 && data[i - 1].code != 0 && data[i - 1].width == 2
