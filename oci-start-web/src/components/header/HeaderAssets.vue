...
<template>
  <el-dialog v-model="visible" class="header-assets-dialog" :title="t('headerAssets.title')" width="min(760px, calc(100vw - 24px))" align-center append-to-body destroy-on-close @close="closeReport">
    <div class="asset-report" :aria-busy="loading || undefined">
      ...
      <div v-else-if="loadError" class="asset-load-error" role="alert"><strong>{{ t('headerAssets.loadFailed') }}</strong><p>{{ headerAssetError(loadError) }}</p></div>
      <template v-else-if="summary">
        ...
        <section class="asset-analysis" :aria-label="t('headerAssets.aiTitle')">
          ...
          <div class="analysis-status" :class="`is-${analysisState}`" role="status" aria-live="polite">
          ...
          <p v-if="analysisError" class="analysis-error" role="alert">{{ headerAssetError(analysisError) }}</p>
...
.asset-load-error { padding: 24px 0; color: var(--status-danger); overflow-wrap: anywhere; }
.asset-load-error strong, .asset-zero strong { font-size: var(--font-size-body); font-weight: 500; }
.asset-load-error p, .asset-zero p { margin: 7px 0 0; font-size: var(--font-size-secondary); line-height: 1.6; }
...
.analysis-status.is-error, .analysis-status.is-unconfirmed, .analysis-error { color: var(--status-danger); }
.analysis-error { margin: 8px 0 0; font-size: var(--font-size-secondary); line-height: 1.6; overflow-wrap: anywhere; }
