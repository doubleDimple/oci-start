...
function menuAction(value: unknown, row: BootTaskGroup) { if (typeof value === 'string' && groupActions.includes(value as typeof groupActions[number])) openAction(value as BootTaskAction, row) }
  ...
  <section class="boot-list-page" :aria-label="t('bootTasks.title')">
    ...
    <div class="list-card">
      ...
      <el-table v-else :data="rows" :row-key="rowKey" :empty-text="loading ? t('bootTasks.loading') : error || t('bootTasks.empty')" :aria-busy="loading" height="100%">
        ...
        <el-table-column v-for="field in countColumns" :key="field" :label="t(`bootTasks.columns.${field}`)" :min-width="field === 'successCount' ? 205 : 135" show-overflow-tooltip><template #header><span :title="field === 'successCount' ? t('bootTasks.successCountHint') : undefined">{{ t(`bootTasks.columns.${field}`) }}</span></template><template #default="{ row }">{{ number(row[field]) }}</template></el-table-column>
        ...
        <el-table-column :label="t('bootTasks.columns.actions')" fixed="right" width="300"><template #default="{ row }"><div class="row-actions">
          <button type="button" :disabled="overlay || loading || !!navigating" @click="details = row as BootTaskGroup">{{ t('bootTasks.actions.details') }}</button>
          <button type="button" :disabled="overlay || loading || !!navigating" :aria-busy="navigating === row.id" @click="addConfiguration(row as BootTaskGroup)">{{ t('bootTasks.actions.add') }}</button>
          <el-dropdown trigger="click" :disabled="overlay || loading || !!navigating" popper-class="boot-task-menu" @command="menuAction($event, row as BootTaskGroup)"><button type="button" :disabled="overlay || loading || !!navigating" :aria-label="t('bootTasks.columns.actions')"><i class="i-mdi-dots-horizontal" aria-hidden="true" /></button><template #dropdown><el-dropdown-menu><el-dropdown-item v-for="action in groupActions" :key="action" :command="action" :class="{ danger: action === 'delete' }" :divided="action === 'delete'">{{ t(`bootTasks.actions.${action}`) }}</el-dropdown-item></el-dropdown-menu></template></el-dropdown>
...
.list-error-notice { margin: 12px 18px; }.list-card :deep(.el-table th.el-table__cell .cell) { color: var(--text-primary); font-size: var(--font-size-body); font-weight: 600; }
...
.icon-button, .row-actions button { border: 1px solid var(--border); border-radius: var(--r-pill); background: var(--bg-card); color: var(--text-primary); font: var(--font-size-body)/1.4 var(--sans); padding: 6px 10px; cursor: pointer; }
.icon-button { display: inline-flex; flex-shrink: 0; border: 0; padding: 6px; }.icon-button i, .row-actions i { width: 18px; height: 18px; }.row-actions { display: flex; align-items: center; gap: 7px; flex-wrap: wrap; }.row-actions button { display: inline-flex; align-items: center; white-space: nowrap; }
.icon-button:hover, .row-actions button:hover { background: var(--bg-hover); }.row-actions button:disabled { opacity: .5; cursor: default; }.icon-button:focus-visible, .row-actions button:focus-visible { outline: 2px solid var(--brand); outline-offset: 2px; }
:global(.boot-task-menu .el-dropdown-menu__item) { color: var(--text-primary); font: var(--font-size-body)/1.5 var(--sans); }:global(.boot-task-menu .el-dropdown-menu__item.danger) { color: var(--status-danger); }
